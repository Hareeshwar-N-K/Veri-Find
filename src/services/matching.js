 ...
 * Returns both the raw score and a fairness-normalized score.
 */
...
/**
 * Fairness-adjusted text similarity.
 * Normalizes the raw Jaccard score to reduce bias against
 * users who write shorter or less detailed descriptions.
 */
function calculateFairTextSimilarity(text1, text2) {
  const raw = calculateTextSimilarity(text1, text2);
  return normalizeDescriptionScore(text1, text2, raw);
}
...
export function calculateMatchScore(lostItem, foundItem) {
  ...
  const scores = {
    ...
    title: calculateFairTextSimilarity(lostItem.title, foundItem.title),
    description: calculateFairTextSimilarity(
  ...
  if (scores.category === 0) return { score: 0, breakdown: scores, biasFlag: null };

  const totalScore = Object.keys(weights).reduce((sum, key) => {
    return sum + scores[key] * weights[key];
  }, 0);

  // Run bias detection on the result
  const biasFlag = detectMatchBias(lostItem, foundItem, scores);
...
export async function findMatchesForLostItem(lostItem, minScore = 0.4) {
  ...
  snapshot.docs.forEach((doc) => {
    ...
    // If score is below threshold but bias was flagged, still include it
    // with a boosted minimum score so it's not silently dropped
    const effectiveScore =
      matchResult.score < minScore && matchResult.biasFlag?.isFlagged
        ? minScore
        : matchResult.score;
...
export async function createMatch(lostItem, foundItem, scoreData) {
  ...
  // Run bias detection on the final match
  const biasFlag = detectMatchBias(lostItem, foundItem, breakdown);
...
export async function verifyMatch(matchId, answers, result) {
  ...
  console.log("Verifying match:", { matchId, answers, result });

  try {
    const matchRef = doc(db, COLLECTIONS.MATCHES, matchId);
    
    // Get current match data to preserve existing verificationQuiz fields
    ...
    const currentQuiz = matchData.verificationQuiz || {};

    // Update the entire verificationQuiz object instead of nested fields
    const updatedQuiz = {
      ...currentQuiz,
      userAnswers: answers,
    ...
    console.log("Match verification updated successfully");
  } catch (error) {
    console.error("Error in verifyMatch:", error);
...
export async function markAsRecovered(matchId, lostItemId, foundItemId) {
  ...
  // Award reputation points to the finder with weekly cap
  if (finderId) {
    try {
      const finderRef = doc(db, COLLECTIONS.USERS, finderId);
      const finderSnap = await getDoc(finderRef);
      const finderData = finderSnap.data() || {};
      const finderWeeklyPoints = finderData.weeklyReputationPoints || 0;

      const { allowed, capped } = applyReputationCap(finderWeeklyPoints, 50);
      ...
      if (allowed > 0) {
        ...
        console.log(`Awarded ${allowed} reputation points to finder: ${finderId}${capped ? " (weekly cap applied)" : ""}`);
      } else {
        console.log(`Finder ${finderId} has reached weekly reputation cap`);
      }
    } catch (error) {
  ...
  // Award points to owner with weekly cap
  if (matchData.ownerId) {
    try {
      const ownerRef = doc(db, COLLECTIONS.USERS, matchData.ownerId);
      const ownerSnap = await getDoc(ownerRef);
      const ownerData = ownerSnap.data() || {};
      const ownerWeeklyPoints = ownerData.weeklyReputationPoints || 0;

      const { allowed, capped } = applyReputationCap(ownerWeeklyPoints, 10);
      ...
      if (allowed > 0) {
        ...
        console.log(`Awarded ${allowed} reputation points to owner: ${matchData.ownerId}${capped ? " (weekly cap applied)" : ""}`);
      }
    } catch (error) {
